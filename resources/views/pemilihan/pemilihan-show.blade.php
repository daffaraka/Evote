...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-center">
                                <img class="img-thumbnail" src="{{ asset('Foto Kandidat/' . $data->foto_kandidat) }}"
                                    alt="">
                            </div>
                            <h2 class="card-title text-center mt-5">{{ $data->nama_pasangan }}</h4>
                                <table>
                                    <tr>
                                        <td>Nama Ketua</td>
                                        <td class="px-3">:</td>
                        ...
                        <div class="d-block px-2">
                            ...
                            @else
                                <form
                                    action="{{ route('pemilihan.vote', ['id_pemilihan' => $pemilihans->id, 'id_kandidat' => $data->id]) }}"
                                    method="POST">
                                    @csrf
    ...
    <script>
        ...
        @foreach ($pemilihans->Kandidat as $pemilihan)

            var kandidatJumlahSuara = [];

            pemilihanLabels.push('{{ $pemilihan->nama_pasangan }}');
            kandidatJumlahSuara.push({{ $pemilihan->jumlah_suara }});
